...
- **Arduino Libararies**: Boards Manager (Arduino AVR Boards), Mouse 1.0.1, Keyboard 1.0.6, USB Host Sheild 2.0 1.7.0.  
- **Python Libraries**: mss, pyserial, numpy, pillow, termcolor, requests, pywin32, opencv-python, serial-tool, pyautogui
---
...
### 2. Software Setup
1. **Install Python**:
   - Download Python 3.14 from [python.org](https://www.python.org/downloads/) or a NEWER version
...
#### Color Settings in `colorant.py`

The purple color used is defined as:

```python
LOWER_COLOR = np.array([140, 110, 150])  # Starting color
UPPER_COLOR = np.array([150, 195, 255])  # Ending color
THRESHOLD = 60  # Brightness threshold
```
You can also try other colors, such as:
...
Yellow color:
```python
LOWER_COLOR = np.array([20, 100, 100])   # Lower threshold: dark yellow
UPPER_COLOR = np.array([35, 255, 255])   # Upper threshold: bright yellow
```
For more accurate color selection, use the online tool: [HSV Color Picker](https://alloyui.com/examples/color-picker/hsv)
...
### 3. Spoof and Upload Firmware to Arduino
1. Run py ./spoofer.py
2. Choose the VID/HID of the mouse you want to spoof, to match the default folder structure.
3. Unplug the mouse and plug it into the USB HOST SHIELD. At this point, the ARDUINO will act as the selected mouse.
### 4. Run the Program
...
2. **Tune Color Thresholds**: Modify HSV values in `main.py` to detect the enemy’s distinctive color (e.g., purple outline).
   ** There are **['grey', 'red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'white']
...
│
└───__pycache__
        colorant.cpython-313.pyc
        mouse.cpython-313.pyc
        screen_cap.cpython-313.pyc
```
